"""
extract_features_baseline.py

Extracts  11 baseline features (asymmetry, border irregularity,
9 colour sub-features) for train / validation / test splits and saves
them as CSV files.
...
def extract_features_for_split(img_paths, labels, mask_dir, split_name="split"):
    """
    Extract all 11 baseline features for one data split.
    ...
    pd.DataFrame
        One row per image with the 11 features plus img_id and is_cancer.
    """
    results = []
    missing = 0
    ...
    for i, (img_path, label) in enumerate(zip(img_paths, labels)):
        ...
        # Asymmetry + border (shape features from mask)
        asym  = asymmetry(mask_img)
        bord  = border_irregularity(mask_img)

        # 9 colour sub-features (from raw image + mask)
